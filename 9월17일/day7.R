...
#xmlValue는 컨텐츠,xmlGetAttr은 속성값
content<- xpathSApply(t,'//*[@id="main-top01-scroll-in"]/div/div/h4/a', xmlValue); 
content

#[:punct:] : 특수문자
content <- gsub("[[:punct:]]", "", content) #찾아서 대체(old,new)
content
...
twitter_token <- create_token(   # 토큰 정보
  app = appname,
...
key <- "취업"
key <- enc2utf8(key)  # 검색어를 utf8로 변환
result <- search_tweets(key, n=100, token = twitter_token)
str(result)
result$retweet_text
content <- result$retweet_text  # 리트윗만
...
gsub("Aa", "", word) 
gsub("(Aa)", "", word) 
gsub("(Aa){2}", "", word);gsub("Aa{2}", "", word) #{}는 숫자 지정, 괄호 안치면 바로 앞의 문자만 인식
# "JAVA javascript Aa 가나다 AAaAaA123 %^&*"
gsub("[Aa]", "", word) #[]안에 들어가면 or(A 또는 a)
gsub("[가-힣]", "", word) 
gsub("[^가-힣]", "", word) 
gsub("[&^%*]", "", word) 
gsub("[[:punct:]]", "", word) 
