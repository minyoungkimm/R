...
getwd() # setwd('xxx') 다른 디렉토리로 바꾸고 싶을 때
...
table(score$result)  # 무조건 factor로 간주함
summary(factor(score$result))  #범주형으로 갯수를 확인하고 싶을 때
score$result = factor(score$result) 
str(score)
summary(score)
score$id = as.character(score$id) #id를 숫자타입으로 보는것은 부적합하기 때문에 문자형으로 변환
