...
# append = TRUE --> 이미 존재하는 테이블에 붙인다
dbWriteTable(conn, "mtcars", mtcars[6:10, ], append = TRUE)
dbReadTable(conn, "mtcars")

# overwrite = TRUE --> 새로운 테이블로 대체한다.
...
lapply(df, sum, na.rm=TRUE)  # 리스트로 반환
sapply(df, sum, na.rm=TRUE)  # 결과가 심플데이터셋
...
# NULL이 리턴 될 때는 sapply가 알아서 list로 반환한다.
